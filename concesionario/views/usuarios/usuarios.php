...
if (!$db_connection) {
    ...
    $selectBranchOffice = oci_parse($db_connection, "SELECT usuario.idusuario, usuario.email, usuario.password, rol.idrol, rol.rol FROM usuario INNER JOIN rol ON usuario.idrol = rol.idrol");
                    ...
                    <tbody>
                        <?php
                            while ($fila = oci_fetch_array($selectBranchOffice, OCI_ASSOC + OCI_RETURN_NULLS)) {
                                print "<tr id='idTr_".$fila["IDUSUARIO"]."' class='idTrClass rolId_".$fila["IDROL"]."'>\n";
                                $boolPrint = false;
