<?php
    ...
    if(!$db_connection){
        ...
        //Ventas
        $selectVenta = oci_parse($db_connection, "SELECT venta.idventa, venta.fecha,
                                                        venta.idcliente, cliente.nombres, cliente.apellidos, cliente.direccion, cliente.nit, cliente.telefono
                                                        FROM Venta
                                                        INNER JOIN cliente ON venta.idcliente = cliente.idcliente");
        $executeVenta = oci_execute($selectVenta);

        //Clientes
        $selectCliente = oci_parse($db_connection, "SELECT cliente.idcliente, cliente.nombres || ' ' || cliente.apellidos
                                                            FROM cliente
                                                            INNER JOIN usuario ON usuario.idusuario = cliente.idusuario
                                                            INNER JOIN rol ON rol.idrol = usuario.idrol");
        $executeCliente = oci_execute($selectCliente);
        $templateCliente = "";

        while($fila = oci_fetch_array($selectCliente, OCI_ASSOC + OCI_RETURN_NULLS)){
            $boolPrint = false;
            foreach($fila AS $key => $elemento){
                if($boolPrint === false){
                    $boolPrint = true;
                    ...
                    $templateCliente .= "<option value='".$fila["IDCLIENTE"]."'>". ($elemento !== null ? htmlentities($elemento, ENT_QUOTES) : "") ."</option>";
                }
            }
        }
        ...
        <script>
            ...
            let templateCliente = "<?= $templateCliente ?>";
        </script>

        <head>
            <title>Ventas</title>
            <link rel="icon" type="image/png" href="../../imgs/ventasIcon.png" />
            <meta charset="utf-8" />
        ...
        <body class="bodyInicio">
            ...
            <div class="tab-content">
                ...
                <div id="cotizacion" class="tab-pane fade">
                    <div class="row">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <h3 class="titleTabPane">Detalle compra</h3>
                            <i><p style ="color: #818080">Puede arrastrar las tarjetas si desea reordenar</p></i>
                        </div>
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12" style="text-align: right;">
                            <button id="btnAddNew" name="btnConfirmarVenta" class="btn btn-success">
                ...
                <div id="ventas" class="tab-pane fade">
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                                <h3 class="titleTabPane">Ventas realizadas</h3>
                            </div>
                        </div>
                        <div class="text-justify marginBottom">
                            ...
                            <div class="tableInfo">
                                <table id="tableVentas" class="tableVentas">
                                    ...
                                    <tbody>
                                        <?php
                                        while ($fila = oci_fetch_array($selectVenta, OCI_ASSOC + OCI_RETURN_NULLS)) {
                                            print "<tr id='idTr_".$fila["IDVENTA"]."' class='idTrClass'>\n";
                                            $boolPrint = false;
                                            $hidden = "";
                                            ...
                                            foreach ($fila AS $key => $elemento) {
                                                ...
                                                if ($key !== "IDCLIENTE") {
                                                    print "<td id='".$key."_".$fila["IDVENTA"]."' $hidden>". ($elemento !== null ? htmlentities($elemento, ENT_QUOTES) : "") ."</td>\n";   
                                                }
                                            }
                                        }
                                        ?>
        ...
        oci_free_statement($selectCliente);
